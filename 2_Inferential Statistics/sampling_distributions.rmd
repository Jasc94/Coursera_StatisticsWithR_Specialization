...
```{r samp1-dist}
samp1 %>% summarise(x_bar = mean(area), sampl_med = median(area), s = sd(area), sampl_iqr = IQR(area), sampl_min = min(area), sampl_max = max(area), sampl_q1 = quantile(area, 0.25), sampl_q3 = quantile(area, 0.75))
# A tibble: 1 x 8
```
...
```{r sampling-dist}
sample_means50 %>% summarise(mu = mean(x_bar), se = sd(x_bar))
...
sample_means_small %>% summarise(count = n())
```
...
samp2 %>% summarise(x_bar = mean(price), sampl_med = median(price), s = sd(price), sampl_iqr = IQR(price), sampl_min = min(price), sampl_max = max(price), sampl_q1 = quantile(price, 0.25), sampl_q3 = quantile(price, 0.75))
```
...
```{r price-sampling}
sample_means50_price <- ames %>% rep_sample_n(size = 50, reps = 5000, replace = TRUE) %>% summarise(x_bar = mean(price))
sample_means50_price %>% summarise(mu = mean(x_bar), se = sd(x_bar))
```
...
sample_means150_price <- ames %>% rep_sample_n(size = 150, reps = 5000, replace = TRUE) %>% summarise(x_bar = mean(price))
ggplot(data = sample_means150_price, aes(x = x_bar)) + geom_histogram()
```
...
samp15 %>% summarise(x_bar = mean(price), med = median(price), s = sd(price), iqr = IQR(price), min = min(price), max = max(price), q1 = quantile(price, 0.25), q3 = quantile(price, 0.75))
```
...
samp_means15_price <- ames %>% rep_sample_n(size = 15, reps = 2000, replace = TRUE) %>% summarise(x_bar = mean(price))
samp_means15_price %>% summarise(mu = mean(x_bar), se = sd(x_bar))
```
...
sample_means150_price <- ames %>% rep_sample_n(size = 150, reps = 2000, replace = TRUE) %>% summarise(x_bar = mean(price))
...
ggplot(data = sample_means50_price, aes(x = x_bar)) + geom_histogram()
ggplot(data = sample_means150_price, aes(x = x_bar)) + geom_histogram()
sample_means50_price %>% summarise(mean = mean(x_bar), se = sd(x_bar))
sample_means150_price %>% summarise(mean = mean(x_bar), se = sd(x_bar))
```
