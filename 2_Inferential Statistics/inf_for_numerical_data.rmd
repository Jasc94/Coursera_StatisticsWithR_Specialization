...
```{r}
ggplot(data = nc, aes(x = mage)) + geom_boxplot()
ggplot(data = nc, aes(x = fage)) + geom_boxplot()
ggplot(data = nc, aes(x = weeks)) + geom_boxplot()
ggplot(data = nc, aes(x = gained)) + geom_boxplot()
ggplot(data = nc, aes(x = weight)) + geom_boxplot()

nc %>% summarise(x_bar = mean(mage), median = median(mage), std = sd(mage), max = max(mage), min = min(mage))
nc %>% summarise(x_bar = mean(fage, na.rm=TRUE), median = median(fage, na.rm=TRUE), std = sd(fage, na.rm=TRUE), iqr = IQR(fage, na.rm=TRUE), max = max(fage, na.rm=TRUE), min = min(fage, na.rm=TRUE))
nc %>% summarise(x_bar = mean(weeks, na.rm=TRUE), median = median(weeks, na.rm=TRUE), std = sd(weeks, na.rm=TRUE), iqr = IQR(weeks, na.rm=TRUE), max = max(weeks, na.rm=TRUE), min = min(weeks, na.rm=TRUE))
nc %>% summarise(x_bar = mean(gained, na.rm=TRUE), median = median(gained, na.rm=TRUE), std = sd(gained, na.rm=TRUE), iqr = IQR(gained, na.rm=TRUE), max = max(gained, na.rm=TRUE), min = min(gained, na.rm=TRUE))
nc %>% summarise(x_bar = mean(weight, na.rm=TRUE), median = median(weight, na.rm=TRUE), std = sd(weight, na.rm=TRUE), iqr = IQR(weight, na.rm=TRUE), max = max(weight, na.rm=TRUE), min = min(weight, na.rm=TRUE))
```
...
ggplot(data = subset(nc, !is.na(nc$habit)), aes(x = habit, y = weight)) + geom_boxplot()

# create two subdatasets for further analysis: nonsmokers and smokers
non_smokers <- subset(nc, nc$habit == 'nonsmoker')
smokers <- subset(nc, nc$habit == 'smoker')

# Histograms to better check skewness
ggplot(data = subset(non_smokers, !is.na(non_smokers$habit)), aes(x = weight)) + geom_histogram()
ggplot(data = subset(smokers, !is.na(smokers$habit)), aes(x = weight)) + geom_histogram()

# IQR for both: nonsmokers and smokers
non_smokers %>% summarise(iqr_ns = IQR(weight))
smokers %>% summarise(iqr_s = IQR(weight))
```
...
# Independence : we can assume the observations were random sampled
# Skewness / size: Both groups have more than 30 observations, so even with skewness
# we can stsill use the CLT
```
...
# This confidence interval is narrower, as expected from a lower level of confidence
```
...
# Since pvalue > significance level, we fail to reject H0 and hence we cannot confirm that there's a difference in the gained weight between young and mature mothers
```

7. Now, a non-inference task: Determine the age cutoff for younger and mature 
mothers. Use a method of your choice, and explain how your method works.
```{r cutoff-mature}
# To determine the cutoff age we can simply find the max value for young mothers and the min value for old mothers
nc %>% group_by(mature) %>% summarise(lower_bound = min(mage), upper_bound = max(mage))
...
```{r pick-your-own}
# Do the pregnancy weeks vary depending on whether or not the mother is smoker?
inference(y = weeks, x = habit, data = nc, statistic = "mean", type = "ht", null = 0, alternative = "twosided", method = "theoretical")

# Since the pvalue is greater than the significance level, we fail to reject the null hypothesis
```
