...
ggplot(data = atheism, aes(x = response)) + geom_bar()
# To see the overall numbers
atheism %>% group_by(response) %>% count()
# To calculate the proportions
atheist_count <- atheism %>% filter(response == "atheist") %>% count()
non_atheist_count <- atheism %>% filter(response == "non-atheist") %>% count()
atheist_prop = atheist_count / (atheist_count + non_atheist_count)
non_atheist_prop = non_atheist_count / (atheist_count + non_atheist_count)
```
...
# using the confidence interval values, we calculate the margin of error
margin_of_error = (0.0634 - 0.0364) / 2
```
...
# Pulling the data for Canada and Australia
canada12 <- atheism %>% filter(nationality == "Canada", atheism$year == "2012")
australia12 <- atheism %>% filter(nationality == "Australia", atheism$year == "2012")
# Constructing confidence intervals for both
inference(y = response, data = canada12, statistic = "proportion", type = "ci", method = "theoretical", success = "atheist")
inference(y = response, data = australia12, statistic = "proportion", type = "ci", method = "theoretical", success = "atheist")
# Calculating margins of error
me_canada = (0.1075 - 0.0898) / 2
me_australia = (0.1183 - 0.0818) / 2
```
...
# I create the subsets for Spain 2005 and 2012
spain05 <- atheism %>% filter(nationality == "Spain", atheism$year == "2005")
spain12 <- atheism %>% filter(nationality == "Spain", atheism$year == "2012")
# I calculate the confidence intervals for both
inference(y = response, data = group_by(spain05), statistic = "proportion", type = "ci", method = "theoretical", success = "atheist")
inference(y = response, data = group_by(spain12), statistic = "proportion", type = "ci", method = "theoretical", success = "atheist")
# For further check
spain05 %>% group_by(response) %>% count()
spain12 %>% group_by(response) %>% count()
prop.test(x = c(115, 103), n = c(1031, 1042), p = NULL, alternative = "two.sided", correct = TRUE)
```
...
# I create the subsets for US 2005 and 2012
us05 <- atheism %>% filter(nationality == "United States", atheism$year == "2005")
us12 <- atheism %>% filter(nationality == "United States", atheism$year == "2012")
# I calculate the confidence intervals for both
inference(y = response, data = group_by(us05), statistic = "proportion", type = "ci", method = "theoretical", success = "atheist")
inference(y = response, data = group_by(us12), statistic = "proportion", type = "ci", method = "theoretical", success = "atheist")
# For further check
us05 %>% group_by(response) %>% count()
us12 %>% group_by(response) %>% count()
prop.test(x = c(10, 50), n = c(992, 952), p = NULL, alternative = "two.sided", correct = TRUE)
```
